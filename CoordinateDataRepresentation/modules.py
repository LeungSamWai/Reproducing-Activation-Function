...
class sin_gaussian_params_uniform(nn.Module):
    def __init__(self, num_features, a, a1, b, b1, c, c1, d, d1):
        super(sin_gaussian_params_uniform, self).__init__()
        ...
        torch.nn.init.uniform_(self.d, a=d, b=d1)

        print('123 sin gaussian uniform std!!!!! great!!! {} {} {} {} {} {} {} {}'.format(a, a1, b, b1, c, c1, d, d1))
...
class sin_poly2_gaussian_params(nn.Module):
    def __init__(self, num_features, a, a1, b, b1, c, c1, d, d1):
        super(sin_poly2_gaussian_params, self).__init__()

        self.a = Parameter(torch.Tensor(1, num_features))
        self.b = Parameter(torch.Tensor(1, num_features))
        self.c = Parameter(torch.Tensor(1, num_features))
        self.d = Parameter(torch.Tensor(1, num_features))
        self.e = Parameter(torch.Tensor(1, num_features))
        self.f = Parameter(torch.Tensor(1, num_features))
        ...
        torch.nn.init.normal_(self.e, mean=1.0, std=0.1)
        torch.nn.init.uniform_(self.f, a=0.01, b=0.05)

    def forward(self, x):
        y = x * self.a.expand_as(x)
        y = y + self.b.expand_as(x) * x ** 2
        y = y + self.c.expand_as(x) * torch.sin(self.d.expand_as(x)*x)
        y = y + self.e.expand_as(x) * torch.exp(-x ** 2 / (2 * (self.f.expand_as(x) ** 2)))
        return y
...
class FCBlock(MetaModule):
    ...
    def __init__(self, in_features, out_features, num_hidden_layers, hidden_features, a, a1, b, b1, c, c1, d, d1,
        ...
        nls_and_inits = {
                         'sin_gaussian_params_uniform': (sin_gaussian_params_uniform, sine_init, first_layer_sine_init),
                         'sin_poly2_gaussian_params': (sin_poly2_gaussian_params, sine_init, first_layer_sine_init),
        ...
        self.net.append(MetaSequential(
            ...
            nl(hidden_features, a, a1, b, b1, c, c1, d, d1) if nonlinearity in ['sin_gaussian_params', 'sin_params', 'sin_poly2_gaussian_params',
                                                                                'sin_poly2_params', 'sin_gaussian_params_uniform', 'gaussian_params_uniform'] else nl
        ))
...
class SingleBVPNet(MetaModule):
    ...
    def __init__(self, out_features=1, type='sine', in_features=2,
                 mode='mlp', hidden_features=256, num_hidden_layers=3, a=0, a1=0, b=0, b1=0, c=0, c1=0, d=0, d1=0, **kwargs):
        super().__init__()
        self.mode = mode
        ...
        self.net = FCBlock(in_features=in_features, out_features=out_features, num_hidden_layers=num_hidden_layers,
                           hidden_features=hidden_features, a=a, a1=a1, b=b, b1=b1, c=c, c1=c1, d=d, d1=d1, outermost_linear=True, nonlinearity=type)
        print(self)
