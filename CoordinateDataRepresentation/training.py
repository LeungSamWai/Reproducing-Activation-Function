        ...
        for epoch in range(epochs):
            ...
            for step, (model_input, gt) in enumerate(train_dataloader):
                ...
                if not total_steps % (steps_til_summary):
                    ...
                    summary_fn(model, model_input, gt, model_output, writer, total_steps)#, save_image = True)
                # elif not total_steps % (steps_til_summary):
                #     # for param_group in optim.param_groups:
                #     #     print('lr', param_group['lr'])
                #     torch.save(model.state_dict(), os.path.join(checkpoints_dir, 'model_current.pth'))
                #     summary_fn(model, model_input, gt, model_output, writer, total_steps)#, save_image=False)
                if not use_lbfgs:
                    optim.zero_grad()
                    train_loss.backward()
                ...
                total_steps += 1
                cur_lr = cosine_lr(optim, lr, total_steps, len(train_dataloader) * epochs)
